...
#iChannel2 "file://./Texturas/Metal/Metal014_1K-JPG_Color.jpg"
#iChannel3 "file://./Texturas/Leather/Leather030.png"

// Uniforms
uniform float     iChannelTime[4];       // Tempo da unidade de canal
//uniform sampler2D iChannel0;             // Canal de textura para a grama
...
vec3 Render(inout vec3 ro, inout vec3 rd, inout vec3 ref, bool last) {
    ...
    if (d.x < MAX_DIST) {
        ...
        vec2 uv = fract(p.xz * 0.5);

        // Substituído pela cor fixa para testes
        if (d.y == float(mat_support)) {
            col = texture(iChannel0, uv).rgb;  // Aplica a textura de grama
        } else if (d.y == float(mat_bar)) {
            col = texture(iChannel2, uv).rgb;  // Aplica a textura da trave
        } else if (d.y == float(mat_ball)) {
            col = texture(iChannel3, uv).rgb;  // Aplica a textura da bola
        }
